...
def plot_kMeans_RGB(X, centroids, idx, K):
    ...
    label_fontsize = 18
    ax.set_xlabel('R value - Redness', fontsize=label_fontsize)
    ax.set_ylabel('G value - Greenness', fontsize=label_fontsize)
    ax.set_zlabel('B value - Blueness', fontsize=label_fontsize)
    # ax.w_yaxis.set_pane_color((0., 0., 0., .2))
    ax.yaxis._axinfo["grid"].update(color=(0., 0., 0., 0.2))
